...
% for luatex-ja and zh_TW fonts setting.
...
% 配合 tikz 繪圖，得先由 lua 做繪固指令的 output。 → 似乎新版本已開始直接支援。
%\usepackage{luacode}

% lualatex 無需以下二行，本身即為 UTF-8 based.
...
% 中文章節（只適合 artice/report/book，memoir 要另外設定）。
...
\usepackage[unicode=true,bookmarks=true,bookmarksnumbered=false,
            bookmarksopen=false,breaklinks=false,pdfborder={0 0 1},
            backref=section,colorlinks=false]{hyperref} % PDF meta-information specification
...
\renewcommand{\printchaptername}{\LARGE\itshape\chaptername} % "Chapter" text font specification
\renewcommand{\printchapternum}{\LARGE\kansuji\thechapter\mbox{ }章\\} % Chapter number font specification % dirty method.
...
%\setlength{\parindent}{1em} % Paragraph indentation 使用一個中文字縮格
